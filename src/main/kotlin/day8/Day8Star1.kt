...
class Day8Star1 {
    ...
    private val nodeRegex = """([A-Z]{3}) = \(([A-Z]{3}), ([A-Z]{3})\)""".toRegex()
    ...
    fun solve() {
        ...
        val nodesMap = inputFileByLine.drop(2).map { nodeRegex.find(it) }
            .associate { it!!.groupValues[1] to Node(it.groupValues[1], it.groupValues[2], it.groupValues[3]) }

        val steps = calculateSteps(nodesMap, instructions)

        println("[Day8Star1] Steps: $steps")
        check(steps == 15871)
    }

    private fun calculateSteps(nodesMap: Map<String, Node>, instructions: String): Int {
        var steps = 0
        var currentStep = 0
