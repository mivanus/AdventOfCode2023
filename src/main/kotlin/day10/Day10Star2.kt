...
class Day10Star2 {
    ...
    fun solve() {
        ...
        val loopNodes = findLoopNodes(inputFileByLine)
        val pipesInsideLoop = findNodesInsideLoop(inputFileByLine, loopNodes)

        println("[Day10Star2] Pipes inside loop: ${pipesInsideLoop.count()}")
        check(pipesInsideLoop.count() == 371)
    }

    private fun findNodesInsideLoop(
        maze: List<String>, loop: Set<Pair<Int, Int>>
    ): Set<Pair<Int, Int>> {
        val enclosedPipes = mutableSetOf<Pair<Int, Int>>()
        val startEndPipes = listOf('|', 'L', 'J', 'S')

        maze.forEachIndexed { i, row ->
            var inLoop = false
            row.indices.forEach { j ->
                val pipe = row[j]
                val isPartOfLoop = loop.contains(Pair(i, j))

                if (isPartOfLoop && startEndPipes.contains(pipe)) {
                    inLoop = !inLoop
                } else if (!isPartOfLoop && inLoop) {
                    enclosedPipes.add(Pair(i, j))
                }
            }
        }
    ...
    private fun findLoopNodes(maze: List<String>): Set<Pair<Int, Int>> {
        ...
        while (positionsToCheck.isNotEmpty()) {
            ...
            positionsToCheck =
                positionsToCheck.flatMap { findConnectedPipes(it, maze) }.filter { !loopNodes.contains(it) }
        }
